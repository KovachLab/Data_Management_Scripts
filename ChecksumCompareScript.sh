...
# find /mnt/lustre/mel/shared/GECO/Data/SparrowRawData/20200728_INVS-SP_LFe_SparrowWholeGenomeShotgun/ -type f \( -not -name Premise-20200728_INVS-SP_LFe_SparrowWholeGenomeShotgun-md5sum-$current_time.txt \) -exec md5sum '{}' \; > Premise-20200728_INVS-SP_LFe_SparrowWholeGenomeShotgun-md5sum-$current_time.txt &
...
#SourceFilename="Premise-20200728_INVS-SP_LFe_SparrowWholeGenomeShotgun-md5sum-2021.02.08-12.59.txt"
...
if  [ $SourceAnswer == "y" ]; then
  ...
  find $SourceLocation -type f \( -not -name $SourceFilename \) -exec md5 '{}' \; > $SourceFilename &
  SourceMD5_PID=$!
...
if  [ $CloudRunAnswer == "y" ]; then
  ...
  find $CloudBackupLocation -type f \( -not -name $CloudFilename \) -exec md5 '{}' \; > $CloudFilename &
  CloudBackupMD5_PID=$!
...
if  [ $HardDriveRunAnswer == "y" ]; then
  ...
  find $HardDriveBackupLocation -type f \( -not -name $HardDriveFilename \) -exec md5 '{}' \; > $HardDriveFilename &
